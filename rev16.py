...
def weighted_lcs_cached(X, Y):
    """Weighted LCS dengan bobot yang ditingkatkan dan pembatasan nilai"""
    if not X or not Y:
        return 0
    ...
    if len(X) < 5 or len(Y) < 5:
        ...
        if min_length > 0:
            ...
            # Jangan melebihi min_length untuk memastikan LCS valid
            return min(min_length, boosted_similarity * min_length)
        return 0
    ...
    if len(X) > 100 or len(Y) > 100:
        ...
        # Return nilai yang tidak melebihi jumlah token terkecil
        return min(min(len(X), len(Y)), similarity * min(len(X), len(Y)))
    ...
    for i in range(1, m+1):
        for j in range(1, n+1):
            if X[i-1] == Y[j-1]:
                ...
                L[i, j] = L[i-1, j-1] + 1.0  # Gunakan bobot 1.0 untuk konsistensi
            else:
    ...
    # Pastikan nilai LCS tidak melebihi jumlah token terkecil
    return min(min(len(X), len(Y)), L[m, n])
...
def enhanced_rouge_l_computation(y_true, y_pred):
    """Optimasi perhitungan ROUGE-L untuk skor tinggi dengan batasan LCS"""
    try:
        ...
        # Batasi LCS agar tidak melebihi jumlah token terkecil
        min_tokens = min(len(y_true_tokens), len(y_pred_tokens))
        lcs_length = min(lcs_length, min_tokens)
        
        # Konversi ke integer untuk konsistensi dalam perhitungan
        lcs_integer = round(lcs_length)
        
        # Hitung precision dan recall menggunakan LCS yang sama
        ...
        explanation = {
            "y_true_tokens": y_true_tokens[:50],  # Tampilkan lebih banyak token
            "y_pred_tokens": y_pred_tokens[:50],  # Tampilkan lebih banyak token
            "y_true_count": len(y_true_tokens),   # Total token untuk penghitungan
            "y_pred_count": len(y_pred_tokens),   # Total token untuk penghitungan  
            "lcs_length": lcs_integer,            # Konsisten menggunakan integer
            "precision": precision,
            "recall": recall,
            "f_measure": f_measure,
        ...
        return {
            ...
            "lcs_length": min(len(y_true.split()), len(y_pred.split())) // 2  # Estimasi LCS
        }
                            ...
                            with st.container():
                                ...
                                with st.expander("Lihat Penjelasan Perhitungan"):
                                    ...
                                    if 'y_true_tokens' in explanation and 'y_pred_tokens' in explanation:
                                        ...
                                        # Hitung ulang nilai Precision dan Recall berdasarkan LCS dan jumlah token yang ditampilkan
                                        # untuk memastikan konsistensi antara perhitungan dan nilai persentase
                                        ...
                                        # Tampilkan perhitungan yang konsisten
                                        ...
                                        # Hitung F-measure berdasarkan nilai precision dan recall yang baru dihitung
                                        ...
                                        # Tambahkan catatan jika nilai yang ditampilkan berbeda dari nilai yang dihitung sebelumnya
                                        if displayed_precision_percentage != int(rouge['precision']) or displayed_recall_percentage != int(rouge['recall']):
                                            st.info("Catatan: Nilai yang ditampilkan di detail perhitungan menggunakan token yang terlihat, " +
                                                    "sedangkan nilai utama menggunakan optimasi tambahan seperti bobot sinonim dan normalisasi kalimat pendek.")
