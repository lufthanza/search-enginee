...
# Fungsi untuk membagi dokumen menjadi kalimat dengan optimasi threading dan caching
def split_into_sentences(doc_texts):
    """Membagi dokumen menjadi kalimat dengan optimasi, menjaga integritas kalimat."""
    start_time = time.time()
    split_texts = {}
    
    def process_document(file_text_tuple):
        file, full_text = file_text_tuple
        ...
        all_sentences = []
        # Robust sentence tokenization: Process the entire text to get sentences first.
        # This is more robust than splitting text into arbitrary character chunks before tokenizing.
        # If the document is extremely large and causes memory issues here,
        # we would need more advanced techniques like lazy loading or external libraries
        # designed for very large text processing, but for most practical cases
        # where the overall text fits in memory, this is best for sentence integrity.
        
        # New approach: Tokenize sentences first, then handle large lists of sentences
        raw_sentences = nltk.sent_tokenize(full_text)
        
        # If the number of raw sentences is extremely high (e.g., millions),
        # consider a further internal chunking strategy, but one that operates on sentence lists
        # rather than raw character counts to preserve sentence integrity.
        
        current_sentence_idx = 1
        for sent in raw_sentences:
            if sent.strip(): # Ensure we don't add empty sentences
                all_sentences.append((current_sentence_idx, sent.strip()))
                current_sentence_idx += 1
    ...
    # Use thread pool for parallel processing of *documents*
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = list(executor.map(process_document, doc_texts.items()))
    ...
    for file, sentences in results:
        ...
        if file not in st.session_state.file_stats:
            st.session_state.file_stats[file] = {
                'sentences': len(sentences),
                'words': sum(len(s[1].split()) for s in sentences),
                'size': len(doc_texts.get(file, '')) # Get original text size for stats
            }
...
def exact_match_search(keyword, split_texts, sentence_index):
    ...
    keyword_terms = keyword_lower.split()  # Uncommented this line to fix the error
...
# Optimasi untuk BM25 search dengan prioritas kesamaan kalimat
# def bm25_search(keyword, split_texts, processed_sentences):
#     """Pencarian BM25 yang dioptimasi untuk menemukan kalimat yang mirip secara kontekstual dengan query"""
#     # Check cache first
...
#     # Proses keyword sebagai satu unit query (tidak dipisah per kata)
...
#         try:
#             # Create BM25 model - setiap kalimat adalah satu dokumen
#             # Tingkatkan parameter k1 untuk memberi bobot lebih pada term frequency
#             # Kurangi parameter b untuk mengurangi penalti dokumen panjang
#             bm25 = BM25Okapi(tokenized_corpus, k1=1.8, b=0.65)
            
#             # Hitung skor untuk query secara keseluruhan
#             scores = bm25.get_scores(query_tokens)
            
#             # Tambahkan faktor kontekstual dan similaritas frasa
#             result_sentences = []
#             for i, (score, sent, sent_idx) in enumerate(zip(scores, corpus_sentences, sentence_indices)):
#                 if score > 0:  # Filter threshold
#                     # Hitung faktor kontekstual
                    
#                     # 1. Exact phrase match - bonus tinggi jika frasa query ada di kalimat
#                     sent_lower = sent.lower()
#                     keyword_lower = keyword_clean.lower()
#                     phrase_bonus = 0.0
                    
#                     if keyword_lower in sent_lower:
#                         phrase_bonus = 2.0  # Bonus tinggi untuk exact phrase match
                    
#                     # 2. Consecutive words match - cek urutan kata yang sama
#                     consecutive_bonus = 0.0
#                     sent_tokens = tokenized_corpus[i]
                    
#                     # Cari urutan kata yang sama (n-gram matching)
#                     max_ngram_match = 0
#                     for n in range(2, min(len(query_tokens), len(sent_tokens)) + 1):
#                         query_ngrams = [tuple(query_tokens[j:j+n]) for j in range(len(query_tokens) - n + 1)]
#                         sent_ngrams = [tuple(sent_tokens[j:j+n]) for j in range(len(sent_tokens) - n + 1)]
                        
#                         common_ngrams = set(query_ngrams).intersection(set(sent_ngrams))
#                         if common_ngrams:
#                             max_ngram_match = n
                    
#                     if max_ngram_match > 1:
#                         consecutive_bonus = max_ngram_match * 0.3  # Bonus untuk setiap kata berurutan yang cocok
                    
#                     # 3. Word order similarity - jika kata-kata muncul dalam urutan yang sama
#                     order_bonus = 0.0
#                     query_set = set(query_tokens)
#                     match_indices = [j for j, token in enumerate(sent_tokens) if token in query_set]
                    
#                     if len(match_indices) > 1:
#                         # Cek apakah urutan match relatif sama dengan query
#                         is_ordered = all(match_indices[j] < match_indices[j+1] for j in range(len(match_indices)-1))
#                         if is_ordered:
#                             order_bonus = 0.5  # Bonus jika urutan kata yang cocok sama
                    
#                     # 4. Posisi match dalam kalimat - match di awal lebih penting
#                     position_bonus = 0.0
#                     if match_indices and len(sent_tokens) > 0:
#                         avg_position = sum(match_indices) / len(match_indices)
#                         normalized_position = 1.0 - (avg_position / len(sent_tokens))
#                         position_bonus = normalized_position * 0.3  # Bonus untuk match di awal kalimat
                    
#                     # 5. Coverage query dalam kalimat
#                     coverage_ratio = len(set(sent_tokens).intersection(set(query_tokens))) / len(set(query_tokens))
#                     coverage_bonus = coverage_ratio * 0.8
                    
#                     # 6. Kesamaan semantik (menggunakan overlapping kata secara proporsional)
#                     semantic_bonus = 0.0
#                     query_length = len(query_tokens)
                    
#                     if query_length > 0:
#                         # Hitung berapa persen dari query yang ter-cover dalam kalimat
#                         matched_query_terms = sum(1 for term in query_tokens if term in sent_tokens)
#                         semantic_match = matched_query_terms / query_length
#                         semantic_bonus = semantic_match * 1.0  # Bonus tinggi untuk kecocokan semantik
                    
#                     # Gabungkan semua faktor dengan bobot yang disesuaikan
#                     context_score = phrase_bonus + consecutive_bonus + order_bonus + position_bonus + coverage_bonus + semantic_bonus
                    
#                     # Final score: kombinasi BM25 dengan kontekstual (beri bobot lebih ke kontekstual)
#                     final_score = score * 0.4 + context_score * 0.6
                    
#                     # Tambahkan hasil dengan skor final dan penjelasan
...
#             # Get top results (tanpa penjelasan)
...
def bm25_search(keyword, split_texts, processed_sentences):
    ...
    # --- Query Preprocessing ---
    # 1. Clean the raw keyword for phrase matching
    raw_keyword_lower = keyword.lower()
    # 2. Preprocess for token-based matching (BM25)
    keyword_clean_for_tokens = advanced_preprocess(keyword)
    # IMPORTANT FIX: Remove stopwords from the query tokens for BM25 calculation
    query_tokens = remove_stopwords(nltk.word_tokenize(keyword_clean_for_tokens))
    
    if not query_tokens: # Handle empty query after stopword removal
        st.warning("Keyword terlalu umum (hanya berisi stopwords) atau terlalu pendek setelah preprocessing.")
        return {}

    # Regex pattern for exact phrase matching (case-insensitive and whole words within the phrase)
    # This remains operating on the original case-insensitive keyword for true "phrase" check
    exact_phrase_regex = re.compile(r'\b' + re.escape(raw_keyword_lower) + r'\b') 

    def process_document(file_sentences_processed):
        file, sentences_list, file_processed = file_sentences_processed
        
        if not file_processed:
            return file, []
            
        # --- First Pass: Identify sentences with an exact phrase match ---
        # This is the primary mechanism to ensure "whole sentence" matches are prioritized.
        exact_match_sentences_data = []
        for idx, sent in sentences_list:
            if exact_phrase_regex.search(sent.lower()):
                exact_match_sentences_data.append((idx, sent))
        
        # Determine the set of sentences to be scored by BM25
        # If exact matches are found, BM25 will *only* be run on those.
        # Otherwise, BM25 runs on all sentences in the file.
        sentences_to_score_by_bm25 = exact_match_sentences_data if exact_match_sentences_data else sentences_list

        corpus_sentences = []
        tokenized_corpus = [] # These tokens are ALREADY stop-word removed from build_sentence_index
        sentence_indices = []
        
        for idx, sent in sentences_to_score_by_bm25:
            processed_data = file_processed.get(idx)
            if processed_data and processed_data['tokens']: # Ensure it was processed and has tokens
                corpus_sentences.append(sent)
                tokenized_corpus.append(processed_data['tokens']) # Use the pre-processed tokens (stop-word removed)
                sentence_indices.append(idx)
        ...
        try:
            bm25 = BM25Okapi(tokenized_corpus, k1=1.8, b=0.65)
            scores = bm25.get_scores(query_tokens) # Now query_tokens are also stop-word removed
            
            result_sentences = []
            for i, (score, sent, sent_idx) in enumerate(zip(scores, corpus_sentences, sentence_indices)):
                if score >= 0: # Include sentences with 0 score, bonuses might lift them
                    sent_lower = sent.lower()
                    
                    # --- Contextual Scoring Factors (Highly Weighted) ---
                    
                    # 1. Exact phrase match - SUPER HIGH BONUS for the full, original keyword phrase
                    phrase_bonus = 0.0
                    if exact_phrase_regex.search(sent_lower):
                        phrase_bonus = 500.0 # Dramatically increased for absolute prioritization
                    
                    # Get tokens without stopwords from pre-processed data for n-gram, order, coverage, semantic checks
                    sent_tokens_no_stop = file_processed[sent_idx]['tokens']
                    
                    # 2. Consecutive words match (n-gram matching on content words)
                    consecutive_bonus = 0.0
                    if len(query_tokens) > 1: # Only relevant for multi-word queries
                        max_ngram_match = 0
                        for n in range(2, min(len(query_tokens), len(sent_tokens_no_stop)) + 1):
                            # N-grams from stopword-removed query and sentence
                            query_ngrams = [tuple(query_tokens[j:j+n]) for j in range(len(query_tokens) - n + 1)]
                            sent_ngrams = [tuple(sent_tokens_no_stop[j:j+n]) for j in range(len(sent_tokens_no_stop) - n + 1)]
                            
                            common_ngrams = set(query_ngrams).intersection(set(sent_ngrams))
                            if common_ngrams:
                                max_ngram_match = n # Keep track of the longest common n-gram
                        if max_ngram_match > 1:
                            consecutive_bonus = max_ngram_match * 2.0 # Significantly increased from 1.0

                    # 3. Word order similarity (on content words)
                    order_bonus = 0.0
                    if len(query_tokens) > 1: # Only relevant for multi-word queries
                        query_matched_terms_in_order = [token for token in query_tokens if token in sent_tokens_no_stop]
                        
                        if len(query_matched_terms_in_order) > 1:
                            current_sent_token_index = -1
                            is_ordered = True
                            for q_term in query_matched_terms_in_order:
                                try:
                                    # Find the next occurrence of q_term in the sentence *after* the previous one
                                    found_idx = sent_tokens_no_stop.index(q_term, current_sent_token_index + 1)
                                    current_sent_token_index = found_idx
                                except ValueError: # If term not found in expected order, break
                                    is_ordered = False
                                    break
                            if is_ordered:
                                order_bonus = 10.0 # Very high bonus for maintaining relative order

                    # 4. Position match (words at the beginning of the sentence)
                    position_bonus = 0.0
                    match_indices = [j for j, token in enumerate(sent_tokens_no_stop) if token in set(query_tokens)]
                    if match_indices and len(sent_tokens_no_stop) > 0:
                        # Reward matches closer to the beginning
                        avg_position = sum(match_indices) / len(match_indices)
                        normalized_position = 1.0 - (avg_position / len(sent_tokens_no_stop))
                        position_bonus = normalized_position * 1.0 # Increased from 0.5

                    # 5. Coverage of query terms in the sentence
                    coverage_ratio = len(set(sent_tokens_no_stop).intersection(set(query_tokens))) / len(set(query_tokens)) if len(query_tokens) > 0 else 0
                    coverage_bonus = coverage_ratio * 50.0 # Huge bonus for covering all query terms

                    # 6. Semantic similarity (proportional overlap, on content words)
                    semantic_bonus = 0.0
                    query_length_no_stop = len(query_tokens)
                    if query_length_no_stop > 0:
                        matched_query_terms_count = sum(1 for term in query_tokens if term in sent_tokens_no_stop)
                        semantic_match_ratio = matched_query_terms_count / query_length_no_stop
                        semantic_bonus = semantic_match_ratio * 50.0 # Huge bonus for high semantic overlap

                    # --- Combine all factors ---
                    # The phrase_bonus will dominate if present. Other bonuses add nuance.
                    context_score = phrase_bonus + consecutive_bonus + order_bonus + position_bonus + coverage_bonus + semantic_bonus
                    
                    # Final score: BM25 contributes minimally, context_score dominates
                    final_score = (score * 0.05) + (context_score * 0.95) # BM25 reduced to 5%, context to 95%
                    
                    # Ensure final score is not negative (BM25 can produce negative scores)
                    ...
                    result_sentences.append((
                        ...
                        sent,
                        {
                            ...
                            'semantic_bonus': semantic_bonus,
                            'context_score': context_score,
                            'final_score': final_score # For debugging/inspection
                        }
                    ))
            ...
            # Get top results (idx, sent)
            top_results = [(idx, sent) for _, idx, sent, _ in result_sentences[:MAX_RESULTS_TO_SHOW]]
            
            return file, top_results
        except Exception as e:
            st.error(f"Error BM25 processing file {file}: {str(e)}\nTraceback: {traceback.format_exc()}")
            return file, []
    
    # Process documents in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = []
        for file, sentences_list in split_texts.items():
            file_processed = processed_sentences.get(file, {})
            futures.append(executor.submit(
                process_document, 
                (file, sentences_list, file_processed)
            ))
